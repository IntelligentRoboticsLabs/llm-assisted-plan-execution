...
{
public:
  using NavigateToPose = plansys2_examples_msgs::action::NavigateToPose;
...
private:
  ...
  void execute(const std::shared_ptr<GoalHandleNavigateToPose> goal_handle)
  {
    ...
    auto pose_start = goal_handle->get_goal()->start_pose.pose;
    tf2::Quaternion q;
    tf2::fromMsg(pose_start.orientation, q);
    ...
    double diff_x = pose_cmd.position.x - pose_start.position.x;
    double diff_y = pose_cmd.position.y - pose_start.position.y;
    ...
    while (rclcpp::ok() && (now() - start).seconds() < duration) {
      RCLCPP_INFO(this->get_logger(), "Navigating %lf / %lf ",
        (now() - start).seconds(), duration);
