...
: plansys2::BtActionNode<plansys2_examples_msgs::action::NavigateToPose>(xml_tag_name, action_name, conf)
{
...
Move::on_tick()
{
  if (status() == BT::NodeStatus::IDLE) {
    ...
    if (waypoints_.find(from) != waypoints_.end()) {
      ...
      std::cerr << "No coordinate for waypoint [" << from << "]" << std::endl;
    }

    geometry_msgs::msg::PoseStamped goal_pos, from_pos;
    ...
    from_pos.header.frame_id = "map";
    from_pos.header.stamp = node->now();
    from_pos.pose.position.x = pose_current.x;
    from_pos.pose.position.y = pose_current.y;
    from_pos.pose.position.z = 0;
    from_pos.pose.orientation = tf2::toMsg(tf2::Quaternion({0.0, 0.0, 1.0}, pose_current.theta));

    goal_.pose = goal_pos;
    goal_.start_pose = from_pos;
  }
...
BT_REGISTER_NODES(factory)
{
  BT::NodeBuilder builder =
    ...
    {
      return std::make_unique<plansys2_replan_example::Move>(
        name, "navigate_to_pose", config);
    };

  factory.registerBuilder<plansys2_replan_example::Move>(
    "Move", builder);
}
